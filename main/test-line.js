...
function addStudent(answer) {
    if (answer === `exit`) {//返回主菜单
        rl.prompt();
        flag = 0;
        interaction();
    }
    else if (answer.split(",").length < 7 && answer.split(",").length > 2) {//返回主菜单
        Student.isIn(answer);
        rl.prompt();
        flag = 0;
        interaction();
        ...
        console.log(`请按正确的格式输入（格式：姓名, 学号, 学科: 成绩, ...）：或输入'exit'返回至菜单`);
        interaction();
    }
}
